...
void run_max_cut_heuristic_on_max_cut_problem(const std::string &instance, const std::string &heuristic,
...
void run_max_cut_heuristic_on_QUBO_problem(const std::string &instance, const std::string &heuristic,
                                           const double optimal_cost) {
    std::cout << "solve QUBO problem " << instance << " with max cut heuristic " << heuristic << "\n";
    MaxCutHeuristic *heur = factory.RunMaxCutHeuristic(heuristic, MaxCutInstance(QUBOInstance(instance)), 1.0, false,
                                                       NULL);
    const mqlib::MaxCutSimpleSolution &sol = heur->get_best_solution();
    test(sol.get_weight() <= optimal_cost + 1e-8);
    delete heur;
...
void run_QUBO_heuristic_on_max_cut_problem(const std::string &instance, const std::string &heuristic,
                                           const double optimal_cost) {
...
void run_QUBO_heuristic_on_QUBO_problem(const std::string &instance, const std::string &heuristic,
                                        const double optimal_cost) {
...
int main() {
    ...
    for (const auto &mc_solver: max_cut_solvers) {
    ...
    for (const auto &qubo_solver: qubo_solvers) {
