...
namespace mqlib {
    ...
    void Random::MultiRouletteWheel(const std::vector<double> &scores, int m,
        ...
        for (u_int64_t ct = 0; ct < scores.size(); ++ct) {
            if (scores[ct] > 0.0) {
                values.emplace_back(-log(RandDouble()) / scores[ct],
                                                        ct);
            }
        }
        ...
        if (static_cast<u_int64_t >(m) < values.size()) {
        ...
        int num_return = std::min<int>(m, static_cast<int>(values.size()));
        indices->clear();
